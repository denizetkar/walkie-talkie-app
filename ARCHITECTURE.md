...
*   **Heartbeat:** The Root emits a periodic heartbeat (~1s). If a node stops receiving this heartbeat (~4s), it assumes the Root is gone, reverts to its own ID, and restarts the convergence process.
...
*   **Starvation Prevention:** The queue logic ensures `CONTROL` packets are prioritized, but forces an `AUDIO` packet through if the audio queue is filling up to prevent starvation.
...
2.  **Encoding:**
    ...
    *   **Packetization:** `[Seq Num (2 bytes)] [Opus Data (variable)]`.
3.  **Output (Speaker):**
    *   **Jitter Buffer:** A `BTreeMap<u16, Vec<u8>>`. This sorts incoming packets by Sequence Number automatically.
...
3.  **Runtime:** `MainActivity` explicitly loads `c++_shared` before loading the Rust library to ensure symbols are resolved.