...
mod real_impl {
    ...
    impl AudioEngine {
        ...
        pub fn push_incoming_packet(&self, data: Vec<u8>) {
            // 1. FAST PATH: Try to send if the channel exists
            {
                let tx_guard = self.incoming_tx.lock().unwrap();
                if let Some(tx) = tx_guard.as_ref() {
                    if let Ok(_) = tx.try_send(data) {
                        return; // Success!
                    }
                    // If try_send failed (full), we drop.
                    // If it failed because disconnected, we might need to restart,
                    // but usually that means we are shutting down.
                    return;
                }
            }
            // Lock is dropped here.

            // 2. SLOW PATH: Channel didn't exist. Start the output stream.
            // We only reach here if the speaker was OFF.
            if let Ok(_) = self.ensure_output_running() {
                // 3. RETRY: Now the channel should exist.
                let tx_guard = self.incoming_tx.lock().unwrap();
                if let Some(tx) = tx_guard.as_ref() {
